...
const f2 = () => {
    let res = a2.find(elem => {
        if(elem > a2_from && elem < a2_to){
            return true;
        }
    })
    document.querySelector('.out-2').innerHTML = res == undefined ? false : res;
}
...
const f3 = () => {
    a3_res = [];
    a3_res = a3.filter(e => e > a3_from && e < a3_to);
...
const f10 = () => {
    ...
    let res = a10_res.reduce((res, elem) => res + elem + ' ', '');
