...
const f1 = () => {
    ...
    document.querySelector('.out-1').innerHTML = a1.includes(input);
}
...
const f2 = () => {
    ...
    document.querySelector('.out-2').innerHTML = a2.includes(input) ? a2.indexOf(input) : a2.includes(input);
}
...
const f4 = () => {
    ...
    let res = '';
    for(let e in a4){
        if(a4[e].includes(input)){
            res += e + ' ';
        }
    }
    document.querySelector('.out-4').innerHTML = res != '' ? res : false;
}
...
const f5 = () => {
    ...
    document.querySelector('.out-5').innerHTML = a5.includes(value, index);
...
const f9 = () => {
    ...
    if(!isNaN(parseInt(input))){
        res = a9.includes(+input);
    } else {
        res = a9.includes(input.toUpperCase());
        if(!res){
            res = a9.includes(input.toLowerCase());
        }
    }
