...
const f4 = (arr, delimiter) => {
    let tmp = arr.pop();
    let res = arr.reduce((r, e) => r + e + delimiter, '');
    res = res + tmp;
    return res;
}
...
const f9 = () => {
    a9_res = [...s9];
    document.querySelector('.out-9').innerHTML = a9_res.reduce((r, e) => r + e + ' ', '');
}
...
const f10 = () => {
    a10_res = [...s10];
    let res = a10_res.filter(e => e < 'l');
    document.querySelector('.out-10').innerHTML = res.reduce((r, e) => r + e + ' ', '');
}
