...
const f2 = () => {
    ...
    //     console.log(a2[i]);
    // }
    let res = Object.prototype.toString.call(a2) == "[object Array]";
...
const f16 = () => {
    let res = {};
    let tmp = '';
    res = a16.reduce((accum, item) => {
        accum[item.id] = item.name;
        tmp += item.id + ' ' + item.name + '<br>';
        return accum;
    }, {});
    console.log(res);
    document.querySelector('.out-16').innerHTML = tmp;
}
...
const f17 = () => {
    ...
    console.log(a17_res);
    document.querySelector('.out-17').innerHTML = a17_res.reduce((acc, item) => acc + item + ' ', '');
}
...
const f18 = () => {
    let res = a18.reduce((accum, item, index) => accum + index + ' ' + item + '<br>', '');
...
const f20 = () => {
    let res = [];
    res = a20.reduce((accum, item) =>  {
        accum.unshift(item);
        return accum;
    }, []);
    console.log(res);
    document.querySelector('.out-20').innerHTML = res.reduce((acc, item) => acc + ' ' + item + ' ', '');
    // res = [6,5,4]
}
